...
arbiter_status()
{
    ...
    # linda 添加PGP
    if [ "$CHAIN_TYPE" == "mainnet" ]; then
        local PGP_GENESIS=08b947e87f2a0ae0e07d81f93619b3f445ed132747c5c1d8e963e4a922856ec3
    elif [ "$CHAIN_TYPE" == "testnet" ]; then
        local PGP_GENESIS=3043bcc03c90a37a292a4357ee972bc392b143e75e1b79205e113688e3bd071b
    else
        echo_error "do not support $CHAIN_TYPE"
        return
    fi
...
{
  local ARBITER_ADD_PGP_CONFIG=${SCRIPT_PATH}/arbiter/add_pgp_config.json
  ...
  cp -v ${SCRIPT_PATH}/arbiter/config.json ${SCRIPT_PATH}/arbiter/config_backup_add_pgp_before_2025_08_01.json
  echo "modify arbiter config file..."
  if [ "$CHAIN_TYPE" == "testnet" ]; then
    echo "add testnet pgp config"
    cat >$ARBITER_ADD_PGP_CONFIG <<EOF
{
        "Rpc": {
          "IpAddress": "127.0.0.1",
          "HttpJsonPort": 20662
...
EOF
    echo $ARBITER_ADD_PGP_CONFIG
    cd ${SCRIPT_PATH}/arbiter
    sed -i '28r add_pgp_config.json' config.json
    rm -rf $ARBITER_ADD_PGP_CONFIG
  else
...
EOF
    echo $ARBITER_ADD_PGP_CONFIG
    cd ${SCRIPT_PATH}/arbiter
    sed -i '26r add_pgp_config.json' config.json
    rm -rf $ARBITER_ADD_PGP_CONFIG
  fi
